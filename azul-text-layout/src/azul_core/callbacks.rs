...
impl Update {
    pub fn max_self(&mut self, other: Self) {
        if *self == Update::DoNothing && other != Update::DoNothing
            || *self == Update::RefreshDom && other == Update::RefreshDomAllWindows
        {
            *self = other;
        }
    }
...
pub struct CallbackInfo {
    ...
    words_changed_in_callbacks: *mut BTreeMap<DomId, BTreeMap<NodeId, String>>,
...
impl CallbackInfo {
    ...
    pub fn new<'a, 'b>(
        ...
        Self {
            ...
            words_changed_in_callbacks: words_changed_in_callbacks
                as *mut BTreeMap<DomId, BTreeMap<NodeId, String>>,
    ...
    fn internal_get_words_changed_in_callbacks<'a>(
        &'a mut self,
    ) -> &'a mut BTreeMap<DomId, BTreeMap<NodeId, String>> {
    ...
    pub fn get_string_contents(&self, node_id: DomNodeId) -> Option<String> {
        self.internal_get_layout_results()
            .get(node_id.dom.inner)?
            .words_cache
            .get(&node_id.node.into_crate_internal()?)
    ...
    pub fn set_string_contents(&mut self, node_id: DomNodeId, new_string_contents: String) {
        if let Some(nid) = node_id.node.into_crate_internal() {
