...
impl GlContextPtr {
    ...
    pub fn shader_source(&self, shader: GLuint, strings: Vec<String>) {
    ...
    pub fn get_uniform_iv(&self, program: GLuint, location: GLint, mut result: &mut [GLint]) {
        unsafe {
            self.get()
                .get_uniform_iv(program, location, result.as_mut_slice())
        }
    }
    pub fn get_uniform_fv(&self, program: GLuint, location: GLint, mut result: &mut [GLfloat]) {
        unsafe {
            self.get()
    ...
    pub fn get_uniform_indices(&self, program: GLuint, names: &[&str]) -> Vec<GLuint> {
        self.get().get_uniform_indices(program, &names).into()
    }
    ...
    pub fn get_integer_v(&self, name: GLenum, mut result: &mut [GLint]) {
        unsafe { self.get().get_integer_v(name, result.as_mut_slice()) }
    }
    pub fn get_integer_64v(&self, name: GLenum, mut result: &mut [GLint64]) {
        unsafe { self.get().get_integer_64v(name, result.as_mut_slice()) }
    }
    pub fn get_integer_iv(&self, name: GLenum, index: GLuint, mut result: &mut [GLint]) {
        unsafe {
            self.get()
                .get_integer_iv(name, index, result.as_mut_slice())
        }
    }
    pub fn get_integer_64iv(&self, name: GLenum, index: GLuint, mut result: &mut [GLint64]) {
        unsafe {
            self.get()
                .get_integer_64iv(name, index, result.as_mut_slice())
        }
    }
    pub fn get_boolean_v(&self, name: GLenum, mut result: &mut [GLboolean]) {
        unsafe { self.get().get_boolean_v(name, result.as_mut_slice()) }
    }
    pub fn get_float_v(&self, name: GLenum, mut result: &mut [GLfloat]) {
    ...
    pub fn get_program_iv(&self, program: GLuint, pname: GLenum, mut result: &mut [GLint]) {
        unsafe {
            self.get()
    ...
    pub fn get_vertex_attrib_iv(&self, index: GLuint, pname: GLenum, mut result: &mut [GLint]) {
        unsafe {
            self.get()
                .get_vertex_attrib_iv(index, pname, result.as_mut_slice())
        }
    }
    pub fn get_vertex_attrib_fv(&self, index: GLuint, pname: GLenum, mut result: &mut [GLfloat]) {
        unsafe {
            self.get()
    ...
    pub fn get_shader_iv(&self, shader: GLuint, pname: GLenum, mut result: &mut [GLint]) {
        unsafe {
            self.get()
    ...
    pub fn get_debug_messages(&self) -> Vec<AzDebugMessage> {
...
impl Drop for VertexBuffer {
    fn drop(&mut self) {
        ...
        let copies = unsafe { (*self.refcount).fetch_sub(1, AtomicOrdering::SeqCst) };
        if copies == 1 {
            self.vao.vertex_layout = VertexLayout { fields: vec![] };
            let _ = unsafe { Box::from_raw(self.refcount as *mut AtomicUsize) };
            self.vao
                .gl_context
...
impl GlShader {
    ...
    pub fn new(
        ...
        gl_context.shader_source(
            ...
            vec![String::from(vertex_shader.to_string())].into(),
        );
        gl_context.compile_shader(vertex_shader_object);
        ...
        gl_context.shader_source(
            ...
            vec![String::from(fragment_shader.to_string())].into(),
        );
        gl_context.compile_shader(fragment_shader_object);
    ...
    pub fn draw(
        ...
        let mut uniform_locations: BTreeMap<String, i32> = BTreeMap::new();
