...
use crate::css::{
    AngleMetric, AngleValue, BackgroundPositionHorizontal, BackgroundPositionVertical, BorderStyle,
    BoxShadowClipMode, ColorU, CombinedCssPropertyType, ConicGradient, CssProperty,
    ...
    LayoutTop, LayoutWidth, LinearColorStop, LinearGradient, PercentageValue, PixelValue,
    PixelValueNoPercent, RadialColorStop, RadialGradient, ScrollbarStyle, Shape, SizeMetric,
    StyleBackfaceVisibility, StyleBackgroundContent, StyleBackgroundPosition,
    ...
    StyleBorderTopStyle, StyleBoxShadow, StyleCursor, StyleFilter, StyleFontFamily, StyleFontSize,
    StyleLetterSpacing, StyleLineHeight, StyleMixBlendMode, StyleOpacity, StylePerspectiveOrigin,
    StyleTabWidth, StyleTextAlign, StyleTextColor, StyleTransform, StyleTransformOrigin,
    StyleWordSpacing,
};
...
pub fn parse_style_text_color<'a>(
    ...
    parse_css_color(input).map(|ok| StyleTextColor { inner: ok })
}
...
pub fn parse_style_border<'a>(input: &'a str) -> Result<StyleBorderSide, CssBorderParseError<'a>> {
    ...
    match second_argument_end {
        ...
        Some(end) => {
            ...
            border_width = parse_pixel_value(first_arg_str).map_err(ThicknessParseError)?;
            let border_style_str = &input[first_arg_end..end];
            border_style =
                parse_style_border_style(border_style_str).map_err(InvalidBorderStyle)?;
    ...
    let border_color = parse_css_color(border_color_str).map_err(ColorParseError)?;
    ...
    match count {
        ...
        5 => {
            ...
            if !is_inset {
                ...
                box_shadow.spread_radius = spread;
            }
...
pub fn parse_style_background_content_multiple<'a>(
    input: &'a str,
) -> Result<Vec<StyleBackgroundContent>, CssBackgroundParseError<'a>> {
...
pub fn parse_style_background_position_multiple<'a>(
    input: &'a str,
) -> Result<Vec<StyleBackgroundPosition>, CssBackgroundPositionParseError<'a>> {
    Ok(split_string_respect_comma(input)
        .iter()
...
pub fn parse_style_background_size_multiple<'a>(
    input: &'a str,
) -> Result<Vec<StyleBackgroundSize>, InvalidValueErr<'a>> {
    Ok(split_string_respect_comma(input)
        .iter()
...
pub fn parse_style_background_repeat_multiple<'a>(
    input: &'a str,
) -> Result<Vec<StyleBackgroundRepeat>, InvalidValueErr<'a>> {
    Ok(split_string_respect_comma(input)
        .iter()
...
pub fn parse_style_filter_vec<'a>(
    input: &'a str,
) -> Result<Vec<StyleFilter>, CssStyleFilterParseError<'a>> {
    let _comma_separated_items = split_string_respect_comma(input);
...
pub fn parse_style_filter<'a>(input: &'a str) -> Result<StyleFilter, CssStyleFilterParseError<'a>> {
    use crate::css::{StyleBlur, StyleColorMatrix, StyleCompositeFilter, StyleFilterOffset};
...
pub fn parse_style_transform_vec<'a>(
    input: &'a str,
) -> Result<Vec<StyleTransform>, CssStyleTransformParseError<'a>> {
    let _comma_separated_items = split_string_respect_comma(input);
    let vec = split_string_respect_comma(input)
        .iter()
...
pub fn parse_gradient<'a>(
    ...
    let _is_conic_gradient = background_type == GradientType::ConicGradient
    ...
    if is_linear_gradient {
        ...
        for next_brace_item in brace_iterator.by_ref() {
        ...
        for next_brace_item in brace_iterator {
...
impl<'a> From<QuoteStripped<'a>> for String {
...
pub fn parse_image<'a>(input: &'a str) -> Result<String, CssImageParseError<'a>> {
...
pub fn parse_linear_color_stop<'a>(
    ...
    let offset = match percentage_str {
        None => None,
        Some(s) => Some(parse_percentage_value(s).map_err(Percentage)?),
    };
...
pub fn parse_radial_color_stop<'a>(
    ...
    let offset = match percentage_str {
        None => None,
        Some(s) => Some(parse_angle_value(s).map_err(Angle)?),
    };
...
pub fn parse_angle_value<'a>(input: &'a str) -> Result<AngleValue, CssAngleValueParseError<'a>> {
    ...
    match input.parse::<f32>() {
        ...
        Err(_e) => Err(CssAngleValueParseError::InvalidAngle(input)),
    }
}
...
pub fn parse_style_tab_width(input: &str) -> Result<StyleTabWidth, PercentageParseError> {
    parse_percentage_value(input).map(|e| StyleTabWidth { inner: e })
}
...
pub fn parse_style_font_family<'a>(
    input: &'a str,
) -> Result<Vec<StyleFontFamily>, CssStyleFontFamilyParseError<'a>> {
    ...
    for font in multiple_fonts {
        ...
        let font = font.trim_matches('\"');
        let font = font.trim();
        fonts.push(StyleFontFamily::System(font.to_string()));
    }
...
#[cfg(test)]
mod css_tests {
    use crate::css::{NormalizedLinearColorStop, NormalizedRadialColorStop, RadialGradientSize};
    ...
    */
    ...
    fn test_parse_style_font_family_1() {
        ...
        let fonts0: Vec<StyleFontFamily> = fonts0.into();
        assert_eq!(
